...
import com.android.rgk.common.util.DisplayUtil;
...
public class TrackDrawUtil {
     ...
     * @param showPoint
     */
    public static void drawFaceTracking(List<YMFace> faces, SurfaceView outputView, float scale_bit,
                                        int cameraId, String fps, boolean showPoint, Context context) {
        ...
        try {
            ...
            for (int i = 0; i < faces.size(); i++) {
                ...
                if (cameraId == (Constant.backCameraLeftRightReverse ? Camera.CameraInfo.CAMERA_FACING_FRONT : Camera.CameraInfo.CAMERA_FACING_BACK)) {
                ...
                int size = DisplayUtil.dip2px(context, 2);
                ...
                if (ymFace.getAge() > 0) {//绘制性别、年龄
                    ...
                    int fontSize = DisplayUtil.dip2px(context, 20);
                ...
                if (showPoint) {//绘制人脸关键点
                    ...
                    size = DisplayUtil.dip2px(context, 2.5f);
                    paint.setStrokeWidth(size);
            ...
            if (!TextUtils.isEmpty(fps)) {//绘制fps
                drawFps(fps, canvas, viewH, context);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
     ...
     * @param fps
     */
    public static void drawFaceLiveness(List<YMFace> faces, SurfaceView outputView, float scale_bit,
                                        int cameraId, String fps, Context context) {
        Canvas canvas = outputView.getHolder().lockCanvas();
        if (canvas == null) {
            return;
        }
     ...
     * @param userMap
     */
    public static void drawFaceRecognition(List<YMFace> faces, SurfaceView outputView, float scale_bit,
                                           int cameraId, String fps, Map<Integer, User> userMap, Context context) {
        Canvas canvas = outputView.getHolder().lockCanvas();
        if (canvas == null) {
            return;
        }
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                float rect_width = rect[2] * scale_bit;
                RectF rectf = new RectF(x1, y1, x1 + rect_width, y1 + rect_width);
                int size = DisplayUtil.dip2px(context, 3);
                paint.setStrokeWidth(size);
                paint.setStyle(Paint.Style.STROKE);
...
//                int smailSize = DisplayUtil.dip2px(ExApplication.getContext(), 1.5f);
...
//                }
                //draw horn
                //paint.setStrokeWidth(size);
                //DLog.d("ymFace.getLiveness(): "+ymFace.getLiveness());
                if (ymFace.getLiveness() == 1) {
...
//                canvas.drawLine(x2, y2 + offset, x2, y2 - length, paint);
                ...
                if (personId > 0 && userMap.containsKey(personId) && ymFace.getLiveness() == 1) {
...
//                paint.setColor(Color.WHITE);
                paint.setStrokeWidth(0);
                paint.setStyle(Paint.Style.FILL);
                int fontSize = DisplayUtil.dip2px(context, 50);
                paint.setTextSize(fontSize);
                paint.setTextAlign(Paint.Align.CENTER);
                canvas.drawText(sb.toString(), 160, 140, paint);
            }
            if (!TextUtils.isEmpty(fps)) {//绘制fps
                drawFps(fps, canvas, viewH, context);
            }
     ...
     * @param viewH
     */
    private static void drawFps(String fps, Canvas canvas, int viewH, Context context) {
        if (paint == null) {
            paint = new Paint();
            paint.setAntiAlias(true);
        }
        ...
        int sizet = DisplayUtil.sp2px(context, Constant.backCameraLeftRightReverse ? 28 : 17);

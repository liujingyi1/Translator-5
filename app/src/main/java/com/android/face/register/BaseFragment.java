...
public abstract class BaseFragment extends Fragment implements CameraController.CameraContext,
        CameraController.PreviewCallback, CameraController.CameraStateCallback {
    ...
    public void onDestroyView() {
        super.onDestroyView();
        mCameraController.closeCamera(CameraController.CAMERA_ID_RGB);
    ...
    private void initCameraMsg() {
        if (iw == 0) {
            SurfaceView previewView = getCameraView(CameraController.CAMERA_ID_RGB);
            int surfaceW = previewView.getLayoutParams().width;
            int surfaceH = previewView.getLayoutParams().height;
    ...
    public void onPreviewFrame(byte[] data) {
        ...
        /*if (camera_count == 20 && !save) {
            save = true;
            final int iw = mCameraHolder.getPreviewSize().width;
            final int ih = mCameraHolder.getPreviewSize().height;
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    CameraUtil.saveFromPreview(yuvData, "/sdcard/test.jpg",
                            mCameraHolder.getPreviewSize().width, mCameraHolder.getPreviewSize().height);
                    save = false;
                }
            }).start();
        }*/

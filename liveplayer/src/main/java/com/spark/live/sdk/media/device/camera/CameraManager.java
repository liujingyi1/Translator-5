    ...
    public void startPreview() {
        synchronized (lock) {
            ...
            if (theCamera != null && !previewing) {
                ...
                theCamera.getCamera().setOneShotPreviewCallback(mOneShotPreviewCallback);
                previewing = true;
            }
        }
...
//            Message.obtain(handler, CAMERA_HANDLE_FRAME, data).sendToTarget();
    ...
    private Camera.PreviewCallback mOneShotPreviewCallback = new Camera.PreviewCallback() {
        ...
        @Override
        public void onPreviewFrame(byte[] data, Camera camera) {
            openedCamera.getCamera().setPreviewCallbackWithBuffer(previewCallback);
            openedCamera.getCamera().addCallbackBuffer(CameraConfigManager.getInstance().getBuffer());
        }
    };

...
public class CameraConfigManager {
    ...
    public void setCameraViewSize(int w, int h){
        mCameraViewW = w;
        mCameraViewH = h;
    }
    ...
    public void setCamera(CameraKeeper theCamera, Context context) {
        if (theCamera != null) {
            ...
            Log.d("sqm", "setCamera-----------------------1");
            //finalPreviewSize = CameraConfigurationUtils.findBestPreviewSizeValue(parameters, desiredPreviewSize);
            if (mCameraViewH > 0) {
                finalPreviewSize = CameraConfigurationUtils.findBestPreviewSize(parameters, mCameraViewW, mCameraViewH);
            }
            Log.d("sqm", "setCamera-----------------------2--"+finalPreviewSize+","+desiredPreviewSize);

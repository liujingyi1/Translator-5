...
public class SimpleLivePulisherEngine extends ISimpleLiveEngine.Stub
        implements CameraController.CameraStateCallback, CameraController.PreviewCallback {

    private static final String TAG = "SLPulisherEngine";
    ...
    public SimpleLivePulisherEngine(Context context, CameraController cameraController, String rtmpUrl) {
        Log.d(TAG, "SimpleLivePulisherEngine");
    ...
    public void onStateChange(CameraDevice cameraDevice, CameraDevice.State oldState, CameraDevice.State newState, int cameraId) {
        if (/*state == CameraDevice.State.PARAMETER_READY
                || */newState == CameraDevice.State.PREVIEW_START) {
            onCameraOpen(cameraDevice);
            start(rtmpUrl);
            mCameraController.addRGBPreviewCallback(this);
        } else if (oldState != newState
                && (newState == CameraDevice.State.PREVIEW_STOP || newState == CameraDevice.State.CLOSE)) {
            onCameraClose();
            mCameraController.removeRGBPreviewCallback(this);
        } else {
            onErrorEvent("The camera not open.");
        }
    }
    ...
    public void onCameraOpen(CameraDevice camera) {
        ...
        VideoConfiguration.getInstance()
                .setBitrate(800)
                .setFps(camera.getFrameRate())
                .setGop(5)
                .setMimeType(VideoConfiguration.VIDEO_AVC)
                .setFrameSize(new Point(size.width, size.height),
                        camera.getCameraOrientationDegree());
...
//            audioManager.startRecorder();
    ...
    public void onVideoData(byte[] data) {
        ...
        CameraConfig cameraConfig = CameraConfig.getInstance(CameraController.CAMERA_ID_RGB);
        ...
        final int facing = cameraConfig.getFacing();
        Point size = cameraConfig.getPreviewSize();
        int rotate = cameraConfig.getOrientationDegree();
    ...
    public void start(String rtmpUrl) {
        Log.d(TAG, "SimpleLivePublisherEngine start(" + rtmpUrl + ")");
        //if(!isCaptureDeviceWorking) {
    ...
    public void resume() {
        //cameraManager.startPreview();
        if (!mCameraController.containStateCallback(CameraController.CAMERA_ID_RGB, this)) {
            mCameraController.openCamera(CameraController.CAMERA_ID_RGB, this);
        }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        Point size = CameraConfigManager.getInstance().getFinalPreviewSize();
        if (size == null) {
            CameraConfigManager.getInstance().setCameraViewSize(width, height);
        }

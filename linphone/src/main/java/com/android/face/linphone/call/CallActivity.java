package com.android.face.linphone.call;
/*
CallActivity.java
...
import com.android.face.linphone.ui.Numpad;
...
public class CallActivity extends Activity implements OnClickListener, SensorEventListener, ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    // delete by David,we don't need P sensor changed event
    /*
    ...
    */
    ...
    private void initUI() {
        ...
        try {
            ...
            Logger.e(TAG, "Bluetooth: Audio routes menu disabled on tablets for now (1)");
        }
        ...
        if (BluetoothManager.getInstance().isBluetoothHeadsetAvailable()) {
            try {
                ...
                Logger.e(TAG, "Bluetooth: Audio routes menu disabled on tablets for now (2)");
            }
        } else {
            try {
                ...
                Logger.e(TAG, "Bluetooth: Audio routes menu disabled on tablets for now (3)");
            }
        }
    ...
    private void refreshInCallActions() {
        ...
        try {
            ...
            Logger.e(TAG, "Bluetooth: Audio routes menu disabled on tablets for now (4)");
        }
    }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.video) {
            ...
            Logger.i(TAG, "[Permission] Camera permission is " + (camera == PackageManager.PERMISSION_GRANTED ? "granted" : "denied"));
            ...
            Logger.i(TAG, "[Permission] Record audio permission is " + (recordAudio == PackageManager.PERMISSION_GRANTED ? "granted" : "denied"));
    ...
    */
    ...
    private void showVideoView() {
        if (!BluetoothManager.getInstance().isBluetoothHeadsetAvailable()) {
            Logger.w(TAG, "Bluetooth not available, using speaker");
            LinphoneManager.getInstance().routeAudioToSpeaker();
            isSpeakerEnabled = true;
        }
        refreshInCallActions();
    ...
    private void toggleSpeaker() {
        ...
        if (isSpeakerEnabled) {
            ...
            Logger.d(TAG, "Toggle speaker off, routing back to earpiece");
            LinphoneManager.getInstance().routeAudioToReceiver();
            speaker.setImageResource(R.drawable.speaker_default);
        }
    }
    ...
    public static Boolean isProximitySensorNearby(final SensorEvent event) {
        ...
        Logger.d(TAG, "Proximity sensor report [" + distanceInCm + "] , for max range [" +
                maxDistance + "]");

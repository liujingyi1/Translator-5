...
import com.android.face.linphone.ui.LinphoneOverlay;
    ...
    */
    class ActivityMonitor implements Application.ActivityLifecycleCallbacks {
        ...
        public synchronized void onActivityCreated(Activity activity, Bundle savedInstanceState) {
            Logger.i(TAG, "Activity created:" + activity);
        ...
        public void onActivityStarted(Activity activity) {
            Logger.i(TAG, "Activity started:" + activity);
        }

        @Override
        public synchronized void onActivityResumed(Activity activity) {
            Logger.i(TAG, "Activity resumed:" + activity);
            if (activities.contains(activity)) {
                mRunningActivities++;
                Logger.i(TAG, "runningActivities=" + mRunningActivities);
                checkActivity();
            }
        }
        ...
        public synchronized void onActivityPaused(Activity activity) {
            Logger.i(TAG, "Activity paused:" + activity);
            if (activities.contains(activity)) {
                mRunningActivities--;
                Logger.i(TAG, "runningActivities=" + mRunningActivities);
                checkActivity();
        ...
        public void onActivityStopped(Activity activity) {
            Logger.i(TAG, "Activity stopped:" + activity);
        }
        ...
        public synchronized void onActivityDestroyed(Activity activity) {
            Logger.i(TAG, "Activity destroyed:" + activity);
            if (activities.contains(activity)) {
    ...
    protected void onBackgroundMode() {
        Logger.i(TAG, "App has entered background mode");
    ...
    protected void onForegroundMode() {
        Logger.i(TAG, "App has left background mode");
    }
    ...
    public void onCreate() {
        ...
        Logger.i(TAG, START_LINPHONE_LOGS);
        ...
        LinphoneManager.getLc().addListener(mListener = new LinphoneCoreListenerBase() {
            ...
            public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
                if (instance == null) {
                    Logger.i(TAG, "Service not ready, discarding call state change to " + state
                            .toString());
                    return;
                }
            ...
            public void registrationState(LinphoneCore lc, LinphoneProxyConfig cfg, LinphoneCore.RegistrationState state, String smessage) {
                if (!mDisableRegistrationStatus) {
                    if (displayServiceNotification() && state == RegistrationState.RegistrationOk) {
                        if (LinphoneManager.getLc().getDefaultProxyConfig() != null && LinphoneManager.getLc().getDefaultProxyConfig().isRegistered()) {
                            // add by David for enable h264 default
                            LinphoneManager.getLc().enableOpenH264(true);
                            // add end
                            sendNotification(IC_LEVEL_ORANGE, R.string.notification_registered);
                        } else {
                            LinphoneManager.getLc().setDefaultProxyConfig(cfg);
                    ...
                    if (state == RegistrationState.RegistrationFailed) {
                        if (LinphoneManager.getLc().getDefaultProxyConfig() != null && !LinphoneManager.getLc().getDefaultProxyConfig().isRegistered() && !LinphoneManager.getLc().getDefaultProxyConfig().getIdentity().equals(cfg.getIdentity())) {
                            LinphoneManager.getLc().refreshRegisters();
                        }
                    }
        ...
        if (Version.sdkStrictlyBelow(Version.API05_ECLAIR_20)) {
            try {
                ...
                Logger.e(TAG, "Couldn't find foreground method:" + e);
            }
        } else {
            try {
                ...
                Logger.e(TAG, "Couldn't find startForeground or stopForeground" + e);
            }
        }
    ...
    public void displayMessageNotification(String fromSipUri, String fromName, String message) {
        ...
        try {
            ...
            Logger.e(TAG, "Cannot parse from address " + e1);
        }
    ...
    void invokeMethod(Method method, Object[] args) {
        try {
            ...
            // Should not happen.
            Logger.w(TAG, "Unable to invoke method:" + e);
    ...
    private void dumpInstalledLinphoneInformation() {
        ...
        if (info != null) {
            Logger.i(TAG, "Linphone version is " + info.versionName + " (" + info.versionCode + ")");
        } else {
            Logger.i(TAG, "Linphone version is unknown");
        }
    }
    ...
    private synchronized void notifyWrapper(int id, Notification notification) {
        if (instance != null && notification != null) {
            ...
            Logger.i(TAG, "Service not ready, discarding notification");
        }
    }
    ...
    public void onTaskRemoved(Intent rootIntent) {
        if (getResources().getBoolean(R.bool.kill_service_with_task_manager)) {
            Logger.d(TAG, "Task removed, stop service");
